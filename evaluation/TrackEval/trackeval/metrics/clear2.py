...
def save_idsw_to_json(switch_data, video_name):
    ...
    output_dir = os.getcwd() + '/output/tracker_to_eval/idsw_train_for_test'
    # output_dir = os.getcwd() + '/output/tracker_to_eval/idsw_val_for_test'
    os.makedirs(output_dir, exist_ok=True)
    file_path = os.path.join(output_dir, f"{video_name}.json")

    with open(file_path, "w") as json_file:
        json.dump(switch_data, json_file, indent=4)
    print("Save IDSW data to", file_path)
        
class CLEAR2(_BaseMetric):
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(
            ...
            if np.any(is_idsw):
                for i in np.where(is_idsw)[0]:
                    ...
                    # id_switches.append({
                    #     "frame": t, # visualize from frame 1
                    #     "gt_id": int(gt_id),  
                    #     "previous_tracker_id": int(prev_id), # predict from id 1
                    #     "current_tracker_id": int(current_id) # predict from id 1
                    # })
